...
class GeradorBoleto
{
  ...
  public function gerar(Boleto $boleto, $PDFClass = NULL)
  {
    ...
    $PDF->Cell(
        190, 4, utf8_decode('Utilize folha A4 (210 x 297 mm) ou carta (216 x 279 mm) - Corte na linha indicada:'), '', 1, 'C'
    );
    ...
    $PDF->Cell(
        30, 5, $boleto->getCedente()->getAgencia() . '.' . $boleto->getBanco()->getPosto() . '.' . sprintf('%05d', $boleto->getCedente()->getConta()), 'BR', 0, 'L'
    );
    ...
    $PDF->Cell(
        60, 5, $boleto->getCedente()->getAgencia() . '.' . $boleto->getBanco()->getPosto() . '.' . sprintf('%05d', $boleto->getCedente()->getConta()), 'B', 1, 'R'
    );
  ...
  public function fbarcode($valor, FPDF $PDF)
  {
    ...
    $PDF->SetX($PDF->GetX() + $fino);
    $PDF->Image(
        Gerador::getDirImages() . '/b.png', $PDF->GetX(), $PDF->GetY(), UnidadeMedida::px2milimetros(1), $altura
    );
